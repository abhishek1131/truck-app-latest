...
import { Truck, Search, Package, AlertTriangle, Grid3X3, ArrowLeft, ShoppingCart, RotateCcw } from "lucide-react"
...
export default function TruckDetailPage() {
  ...
  const getBackLabel = () => {
    ...
    return "Back to Trucks"
  }

  // Mock truck data
  const truck = {
    id: truckId,
    name: `Truck #${truckId.padStart(3, "0")}`,
  ...
  const filteredBins = bins.filter(
    (bin) =>
      ...
      bin.categories.some((cat) => cat.toLowerCase().includes(searchTerm.toLowerCase())),
  )

  return (
    <Navigation title={truck.name} subtitle={`${truck.location} • ${truck.status}`}>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <CardContent className="p-4 md:p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl md:text-3xl font-bold text-gray-900">{truck.totalItems}</p>
                  <p className="text-xs md:text-sm text-gray-500">In this truck</p>
                </div>
          ...
          <Card>
            <CardContent className="p-4 md:p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-gray-600">Storage Bins</p>
                  <p className="text-2xl md:text-3xl font-bold text-gray-900">{truck.bins}</p>
                  <p className="text-xs md:text-sm text-gray-500">Compartments</p>
                </div>
            ...
            <CardContent className="p-4 md:p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl md:text-3xl font-bold text-gray-900">{truck.lowStockItems}</p>
            ...
            <CardContent className="p-4 md:p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-lg md:text-xl font-bold text-green-600">{truck.status}</p>
                  <p className="text-xs md:text-sm text-gray-500">Updated {truck.lastUpdated}</p>
                </div>
        ...
        <Card>
          <CardContent className="p-4 md:p-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              ...
              <StandardInventoryModal truckId={truckId} onStandardLevelsSet={handleStandardLevelsSet} />
            </div>
          </CardContent>
        </Card>
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1 mt-2">
                        {bin.categories.map((category) => (
                          <Badge key={category} variant="secondary" className="text-xs">
                            {category}
                          </Badge>
                        ))}
                      </div>
                      <p className="text-xs text-gray-400 mt-1">Updated {bin.lastUpdated}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col md:flex-row items-start md:items-center space-y-3 md:space-y-0 md:space-x-6">
                    <div className="flex space-x-4 md:space-x-6">
                      <div className="text-center">
                        <p className="text-lg md:text-xl font-bold text-gray-900">{bin.totalItems}</p>
                        <p className="text-xs text-gray-500">Items</p>
                      </div>
                      <div className="text-center">
                        <p
                          className={`text-lg md:text-xl font-bold ${bin.lowStockItems > 0 ? "text-red-600" : "text-green-600"}`}
                        >
                          {bin.lowStockItems}
                        </p>
        ...
        {filteredBins.length === 0 && (
          <Card>
            <CardContent className="p-8 text-center">
              ...
              <p className="text-gray-500">Try adjusting your search terms or add a new bin.</p>
            </CardContent>
          </Card>
        )}
      </div>
